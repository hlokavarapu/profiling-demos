...
public:
                 ...
                 const parallel::distributed::Vector<number> &src) const
  {
    ...
    unsigned int eid=eventlog.log_begin("smooth");
    data.cell_loop (&LaplaceOperator::local_apply,
                    this, dst, src);
    eventlog.log_end(eid);
  ...
  void vmult_interface_down(parallel::distributed::Vector<number> &dst,
                            const parallel::distributed::Vector<number> &src) const
  {
    EventLog::Scope e(eventlog, "vmult_interface_down");
    Assert(src.partitioners_are_globally_compatible(*data.get_dof_info(0).vector_partitioner), ExcInternalError());
    Assert(dst.partitioners_are_globally_compatible(*data.get_dof_info(0).vector_partitioner), ExcInternalError());
  ...
  void vmult_interface_up(parallel::distributed::Vector<number> &dst,
                          const parallel::distributed::Vector<number> &src) const
  {
    EventLog::Scope e(eventlog, "vmult_interface_up");
    Assert(src.partitioners_are_globally_compatible(*data.get_dof_info(0).vector_partitioner), ExcInternalError());
    Assert(dst.partitioners_are_globally_compatible(*data.get_dof_info(0).vector_partitioner), ExcInternalError());
  ...
  {
    EventLog::Scope e(eventlog, "initialize_dof_vector");
...
public:
  ...
  virtual void operator() (const unsigned int   level,
                           ...
                           const parallel::distributed::Vector<Number> &src) const
  {
    EventLog::Scope e(eventlog, "coarse");
...
{
  EventLog::Scope e(eventlog, "do_test");

  deallog << "Testing " << dof.get_fe().get_name();
  deallog << std::endl;
  deallog << "Number of degrees of freedom: " << dof.n_dofs() << std::endl;
  deallog << "active cells: " << dof.get_triangulation().n_global_active_cells() << std::endl;
  ...
  {
    EventLog::Scope e(eventlog, "init");
...

  {
    ...
    EventLog::Scope e(eventlog, "vmult");
    preconditioner.vmult(sol, in);
  }
}
...
void test ()
{
  ...
  if (false)
    {
      ...
      tria.refine_global(12-3*dim+2);
      const unsigned int n_runs = fe_degree == 1 ? 6-dim : 5-dim;
      for (unsigned int i=0; i<n_runs; ++i)
        {
          deallog << "run " << i << std::endl;
          eventlog.log_once("run");
    ...
    {
      // conrad mesh 7,
      //refinements = 6: 4,229,471 active cells
      // refinements = 5: 538,063 active cells
      unsigned int refinements = 5;
      const double left =  -1.0;
      const double right = 1.0;
      GridGenerator::subdivided_hyper_cube(tria, 16, left, right);
      tria.refine_global(1);

      for (typename Triangulation<dim>::active_cell_iterator cell=tria.begin_active(); cell != tria.end(); ++cell)
        if (cell->is_locally_owned() &&
            std::abs(cell->center()[0]) >= 0.5 &&
            std::abs(cell->center()[1]) >= 0.5 &&
            std::abs(cell->center()[2]) >= 0.5)
          cell->set_refine_flag ();
      tria.execute_coarsening_and_refinement();
      for (typename Triangulation<dim>::active_cell_iterator cell=tria.begin_active(); cell != tria.end(); ++cell)
        if (cell->is_locally_owned() &&
  ...
  unsigned int id=eventlog.log_begin("distribute");
  dof.distribute_dofs(fe);
  eventlog.log_end(id);
  id=eventlog.log_begin("distribute_mg");
...
int main (int argc, char **argv)
{
  ...
  eventlog.reset((std::string("event.json.")+Utilities::to_string(myid,2)).c_str());
  ...
  unsigned int eid;//=eventlog.log_begin("main");
  test<3,2,double>();
  //test<2,3,double>();
  //test<2,3,float>();
  //eventlog.log_end(eid);
  ...
  for (unsigned int l=0;l<3;++l)
    {
      //eid=eventlog.log_begin("A");
      //test<3,1,double>();
      //test<3,2,float>();
      //eventlog.log_end(eid);

      eid=eventlog.log_begin("test1");
      if (myid==0)
        system("sleep 3");
      eventlog.log_end(eid);
    }
