...
namespace Step40
{
  ...
  void LaplaceProblem<dim>::assemble_system ()
  {
    ...
    test_timer.stop();
    double time = test_timer();
    Utilities::MPI::MinMaxAvg data = Utilities::MPI::min_max_avg(time, MPI_COMM_WORLD);
    pcout << "test min=" << data.min << " max=" << data.max << " avg=" << data.avg << std::endl;
